...
def mlp_optuna(X, y, n_splits=5, test_size=0.2, n_trials=100, epochs = 100):
    ...
    def objective(trial):
        ...
        units_per_layer   = [trial.suggest_int(f"units_{i}", 32, 256, step=32) for i in range(n_mlp_layers)]
        dropout_per_layer = [trial.suggest_float(f"dropout_{i}", 0.1, 0.5) for i in range(n_mlp_layers)]
        ...
        for train_idx, val_idx in tscv.split(X_cv):
            ...
            model = build_mlp(n_features=n_features, n_mlp_layers=n_mlp_layers,
                               dropout_rate=dropout_per_layer, dense_units=units_per_layer,
                                 learning_rate=learning_rate,activation='relu')
    ...
    units_per_layer   = [p[f"units_{i}"]   for i in range(p["n_mlp_layers"])]
    dropout_per_layer = [p[f"dropout_{i}"] for i in range(p["n_mlp_layers"])]

    best_model = build_mlp(n_features=n_features, n_mlp_layers=p["n_mlp_layers"],
                       dense_units=units_per_layer, dropout_rate=dropout_per_layer,
                       learning_rate=p["learning_rate"], activation='relu')
