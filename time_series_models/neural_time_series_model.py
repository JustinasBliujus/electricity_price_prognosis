...
class NeuralTimeSeriesModel(BaseTimeSeriesModel):
    def __init__(self, 
        ...
        super().__init__(feature_names=None,n_splits=n_splits, test_size=test_size,output_dir=output_dir)
    ...
    def fit_fold(self, X_train, y_train, X_val, y_val):
        ...
        model.fit(X_train_s, y_train_s, epochs=self.epochs, batch_size=self.batch_size,validation_data=(X_val_s, y_val_s),callbacks=[EarlyStopping(patience=5, restore_best_weights=True)],verbose=0)
    ...
    def fit_final_24(self, X_train, y_train):
        X_cv_s = self.X_scaler.transform(X_train)
        early_stopping = EarlyStopping(monitor="val_loss",patience=5,restore_best_weights=True)
        y_cv_s = self.y_scaler.transform(y_train.reshape(-1, 1)).flatten()
        model = self.build_model(X_cv_s.shape[-1])
        model.fit(X_cv_s,y_cv_s,epochs=self.epochs,batch_size=self.batch_size,validation_split=0.1,callbacks=[early_stopping],verbose=0,shuffle=False)
        self.model = model