...
class LSTMModel(NeuralTimeSeriesModel):
    ...
    def fit_final_24(self, X_train, y_train_shifted):
        X_cv_s = self.X_scaler.transform(X_train)
        y_shifted_s = self.y_scaler.transform(y_train_shifted.reshape(-1, 1)).flatten()
        X_cv_s, _ = self.create_sequences(X_cv_s, y_shifted_s)
        y_shifted_s = y_shifted_s[self.time_steps - 1:]

        model = self.build_model(X_cv_s.shape[-1])
        model.fit(X_cv_s, y_shifted_s, epochs=self.epochs, batch_size=self.batch_size, verbose=0)
        self.model = model
    ...
    def suggest_hyperparams(self, trial):
        ...
        self.time_steps = trial.suggest_categorical("time_steps", [1,2,3,6,12,24,48,96,168])
